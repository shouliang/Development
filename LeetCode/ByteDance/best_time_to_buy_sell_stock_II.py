...
思路：贪心法：既然能买卖任意次，那最大收益的方法就是尽可能多的低入高抛。只要明天比今天价格高，就应该今天买入明天再卖出。
'''
    ...
    def maxProfit(self, prices):
        ...
        for i in range(1, len(prices)):
            if prices[i] > prices[i - 1]:
                maxProfit += prices[i] - prices[i - 1]  # 贪心算法：只要明天比今天价格高，就应该今天买入明天再卖出。
        return maxProfit
...
class Solution2:
    def maxProfit(self, prices):
        ...
        :rtype: int
        """
        if not prices: return 0      # 错误处理
        dp = [[0, 0]] * len(prices)  # 初始化
        dp[0][0] = 0                 # 第0天没有股票，收益为0
        dp[0][1] = -prices[0]        # 第0天有股票，  收益为负数
        for i in range(1, len(prices)):
            dp[i][0] = max(dp[i - 1][0], dp[i - 1][1] + prices[i])    # 第i天没有股票：取（第i-1天也没有股票） 和 （第i-1天有股票并卖出的收益之和，因为卖出故收益需要加上当天股票价格）的最大值
            dp[i][1] = max(dp[i - 1][1], dp[i - 1][0] + (-prices[i])) # 第i天有股票 : 取（第i-1天也有股票）   和 （第i-1天没有股票并买入的收益之和，因为买入故收益需要减去当天股票价格）的最大值
