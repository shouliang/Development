...
class Solution:
    ...
    def DFS(self, candidates, cur, target, path, result):
        ...
        for i in range(cur, len(candidates)):  # 从这个数的后面即：i + 1 开始进行搜索
            if i > cur and candidates[i] == candidates[i - 1]: continue  # 同一层中，后面的数和前面相同的时候，会出现重复，直接continue。
            path.append(candidates[i]) # 向path中加入元素
            self.DFS(candidates, i + 1, target - candidates[i], path, result)  # 递归遍历下一层，每次修改target
