...
var hexStr string = "fee9ecaadafeee72d2eb66a0bd344cdd"
...
func TestGetBytes(t *testing.T) {
	str := "8d8d9b85e36b2b9486becd31288e9dc2501cf77f95deb7d141eeb49d77f8a80f"
	t.Log(getBytes(str))
}
...
var hkdfTests = []hkdfTest{
	...
	{
		sha3.New256,
		getBytes("8d8d9b85e36b2b9486becd31288e9dc2501cf77f95deb7d141eeb49d77f8a80f"),
		[]byte("LIBRA WALLET: master key salt$"),
		getBytes("16274c9618ed59177ca948529c1884ba65c57984d562ec2b4e5aa1ee3e3903be"),
		[]byte("LIBRA WALLET: derived key$1"),
		[]byte{
...
func TestHKDF(t *testing.T) {
	for i, tt := range hkdfTests {
		...
		// encodedStr := hex.EncodeToString(prk)
		// t.Log(encodedStr)

		hkdfff := hkdf.New(tt.hash, tt.master, tt.salt, tt.info)
		out := make([]byte, 32)
		// t.Log("tt.out is", tt.out)

		n, err := io.ReadFull(hkdfff, out)
		...
		hkdfff = hkdf.Expand(tt.hash, prk, tt.info)
...
func TestHKDFMultiRead(t *testing.T) {
	for i, tt := range hkdfTests {
		hkdf := hkdf.New(tt.hash, tt.master, tt.salt, tt.info)
...
func benchmarkHKDFStream(hasher func() hash.Hash, block int, b *testing.B) {
	...
	hkdfff := hkdf.New(hasher, master, salt, info)
	for i := 0; i < b.N; i++ {
		_, err := io.ReadFull(hkdfff, out)
		if err != nil {
			hkdfff = hkdf.New(hasher, master, salt, info)
			i--
		}
