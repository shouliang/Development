// flag使用 : 命令行参数的变量类型有： 指针类型、值类型
package main

import (
	"flag"
	"fmt"
)

// 定义命令行参数对应的变量，这三个变量都是指针类型
var cliName = flag.String("name", "nick", "Input Your name")
var cliAge = flag.Int("age", 28, "Input Your Age")
var cliGender = flag.String("gender", "male", "Input Your Gender")

// 定义一个值类型的命令行参数变量，在myInit()函数中对其初始化
// 因此，命令行参数对应的变量的定义和初始化是可以分开的
var cliFlag int

func myInit() {
	flag.IntVar(&cliFlag, "flagname", 111222, "Just for demo")
}

func main() {
	myInit()

	// 把用户传递的命令行参数解析为对应变量的值
	flag.Parse()

	// flag.Args() 返回没有被解析的命令行参数
	// flag.NArgs() 返回没有被解析的命令行参数的个数
	fmt.Printf("args=%s, num=%d\n", flag.Args(), flag.NArg())
	for i := 0; i < flag.NArg(); i++ {
		fmt.Printf("args[%d]=%s\n", i, flag.Arg(i))
	}

	// 输出命令行参数
	fmt.Println("name=", *cliName)
	fmt.Println("age=", *cliAge)
	fmt.Println("gender=", *cliGender)

	fmt.Println("flagname=", cliFlag)
}
